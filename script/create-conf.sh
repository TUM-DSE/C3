...
CONFIG_FILE="${CONFIG_DIR}/benchmarks.conf"
COMMANDS_FILE="${CONFIG_DIR}/commands.conf"

# Protocols to generate commands for
PROTOCOLS=("MESI_unord" "MESI_unord_CXL" "MESI_CXL_MOESI" "MESI_CXL_MESIF")

# Output base directory for fig10 experiments (relative to REPO_ROOT)
OUTPUT_BASE="data/fig_10/gem5.output"
...
#------------------------------------------------------------------------------
echo "# FORMAT: suite | name | cores | switched | dir | command" > "${CONFIG_FILE}"
echo "# All paths are relative to C-3-Artifact/benchmarks/" >> "${CONFIG_FILE}"
echo "# switched: Yes = use --switched flag, No = no --switched flag" >> "${CONFIG_FILE}"
...
#------------------------------------------------------------------------------
echo "# FORMAT: suite | protocol | name | command_line" > "${COMMANDS_FILE}"
echo "# All paths are relative to C-3-Artifact/" >> "${COMMANDS_FILE}"
echo "# Output directory: \${REPO_ROOT}/${OUTPUT_BASE}/{suite}/{name}/{protocol}" >> "${COMMANDS_FILE}"
...
for app_dir in "${BENCHMARKS_DIR}/parsec-benchmark/pkgs/apps/"*/inst/amd64-linux.gcc/bin \
    ...
    # Replace {DIR} placeholder with the actual benchmark directory path
    args_for_conf="${args_template//\{DIR\}/benchmarks/${rel_path}}"
    args_for_conf="${args_for_conf//\{OUTDIR\}/output}"  # placeholder for conf file
    
    echo "parsec | ${name} | ${cores} | ${switched} | ${rel_path} | ${binary} ${args_for_conf}" >> "${CONFIG_FILE}"
    
    # Generate full command line for each protocol
    for protocol in "${PROTOCOLS[@]}"; do
        switched_flag=""
        [[ "$switched" == "Yes" ]] && switched_flag="--switched"
        out_dir="${OUTPUT_BASE}/parsec/${name}/${protocol}"
        
        # Replace placeholders in args
        args="${args_template//\{DIR\}/benchmarks/${rel_path}}"
        args="${args//\{OUTDIR\}/${out_dir}}"
        
        # Use --redirect-stdout -e to redirect stderr to simerr.txt for monitoring
        cmd_line="gem5/build/X86_${protocol}/gem5.opt --outdir=${out_dir} --redirect-stdout --redirect-stderr setup/setup.py -c ${cores} ${switched_flag} -ro -- benchmarks/${rel_path}/${binary} ${args}"
        echo "parsec | ${protocol} | ${name} | ${cmd_line}" >> "${COMMANDS_FILE}"
    done
done
...
# Input files for stdin redirect (empty = no input file needed)
...
if [[ -d "${SPLASH_DIR}" ]]; then
    for bench_dir in "${SPLASH_DIR}"/*/; do
        ...
        if [[ -n "$input_file" ]]; then
            echo "splash | ${name} | ${cores} | ${switched} | ${rel_path} | ${args} | ${input_file}" >> "${CONFIG_FILE}"
        else
            echo "splash | ${name} | ${cores} | ${switched} | ${rel_path} | ${args}" >> "${CONFIG_FILE}"
        fi
        ...
        for protocol in "${PROTOCOLS[@]}"; do
            switched_flag=""
            [[ "$switched" == "Yes" ]] && switched_flag="--switched"
            out_dir="${OUTPUT_BASE}/splash/${name}/${protocol}"
            
            if [[ -n "$input_file" ]]; then
                # With input file - need shell to handle stdin redirect
                cmd_line="gem5/build/X86_${protocol}/gem5.opt --outdir=${out_dir} --redirect-stdout --redirect-stderr setup/setup.py -c ${cores} ${switched_flag} -ro -- benchmarks/${rel_path}/${args} < benchmarks/${rel_path}/${input_file}"
            else
                cmd_line="gem5/build/X86_${protocol}/gem5.opt --outdir=${out_dir} --redirect-stdout --redirect-stderr setup/setup.py -c ${cores} ${switched_flag} -ro -- benchmarks/${rel_path}/${args}"
            fi
            echo "splash | ${protocol} | ${name} | ${cmd_line}" >> "${COMMANDS_FILE}"
        done
    done
fi
...
if [[ -d "${PHOENIX_DIR}" ]]; then
    for bench_dir in "${PHOENIX_DIR}"/*/; do
        ...
        echo "phoenix | ${name} | ${cores} | ${switched} | ${rel_path} | ${args}" >> "${CONFIG_FILE}"
        
        # Generate full command line for each protocol
        for protocol in "${PROTOCOLS[@]}"; do
            switched_flag=""
            [[ "$switched" == "Yes" ]] && switched_flag="--switched"
            out_dir="${OUTPUT_BASE}/phoenix/${name}/${protocol}"
            cmd_line="gem5/build/X86_${protocol}/gem5.opt --outdir=${out_dir} --redirect-stdout --redirect-stderr setup/setup.py -c ${cores} ${switched_flag} -ro -- benchmarks/${rel_path}/${args}"
            echo "phoenix | ${protocol} | ${name} | ${cmd_line}" >> "${COMMANDS_FILE}"
        done
    done
fi
