...
void* runSB(void* arg)
{
    ...
    {
        // Thread 0: Write a 1, then read b
        for (int i = tid; i < DEF_RUNS; i+=DEF_THREADS)
        {
            ...
            // First write a=1 using atomic with relaxed ordering
...
{
    unsigned res_cpu0_gpu0 = 0;  // a:0 and b:0 (forbidden outcome for SB test)
    unsigned res_cpu1_gpu0 = 0;  // a:1 and b:0
    unsigned res_cpu0_gpu1 = 0;  // a:0 and b:1
    unsigned res_cpu1_gpu1 = 0;  // a:1 and b:1 
    ...
    std::cout << "Store Buffer (SB) Litmus Test Results with RELAXED ORDERING:" << std::endl;
    ...
    if (res_cpu0_gpu0 > 0) {
        std::cout << "=========================================================================" << std::endl;
        std::cout << "    FORBIDDEN BEHAVIOR OBSERVED: a:0 and b:0 in " << res_cpu0_gpu0 << " cases!" << std::endl;
        std::cout << "    This indicates a causal cycle in memory operations" << std::endl;
        std::cout << "=========================================================================" << std::endl;
        return 2;  // Forbidden outcome was observed
    } else {
        std::cout << "=========================================================================" << std::endl;
        std::cout << "    No forbidden behavior observed (a:0 and b:0)" << std::endl;
...
int main (int argc, char** argv)
{
    ...
    printf("Starting %d threads for Store Buffer (SB) test...\n", thread_count);
    printf("This test is DESIGNED to expose the forbidden outcome (a:0 and b:0)\n");
    ...
    long long int local = (long long int)runSB((void*)0);
    printf("local[0] = %lld\n", local);
