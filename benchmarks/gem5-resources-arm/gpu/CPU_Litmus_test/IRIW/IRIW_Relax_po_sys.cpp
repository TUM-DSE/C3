	...
	for (int i = tid; i < DEF_RUNS; i+=DEF_THREADS)
	{
		//SET A
	       ...
	       auto tmp_a = (std::atomic<int>*) &((var+i)->a);
    		tmp_a->store(1, std::memory_order_release);
	}
}
else if (tid / DEF_THREADS == 1)
{
	for (int i = (tid % DEF_THREADS); i < DEF_RUNS; i+=DEF_THREADS)
	{
		// Load AB
		auto tmp_a = (std::atomic<int>*) &((var+i)->a);
    		(cpu_res0+i)->r0 = tmp_a->load(std::memory_order_acquire);
    		...
    		(gpu_res0+i)->r0 = tmp_b->load(std::memory_order_acquire);
	}
...
else if (tid / DEF_THREADS == 2)
{
	for (int i = (tid % DEF_THREADS); i < DEF_RUNS; i+=DEF_THREADS)
	{
		...
		for (long long int j = 0; j < ((*(cpu_rand2+i))); j++);
    		...
    		for (long long int j = 0; j < ((*(cpu_rand2+i))); j++);
    		auto tmp_b = (std::atomic<int>*) &((var+i)->b);
    		tmp_b->store(1, std::memory_order_release);
	}
}
else
{
	for (int i = (tid % DEF_THREADS); i < DEF_RUNS; i+=DEF_THREADS)
	{
		//LOAD BA
		auto tmp_b = (std::atomic<int>*) &((var+i)->b);
		(gpu_res1+i)->r0 = tmp_b->load(std::memory_order_acquire);
		for (int j = 0; j < (*(gpu_rand2+i)); j++);
    		...
    		for (int j = 0; j < (*(gpu_rand2+i)); j++);
    		auto tmp_a = (std::atomic<int>*) &((var+i)->a);
    		(cpu_res1+i)->r0 = tmp_a->load(std::memory_order_acquire);
	}	
}
