...
parser.add_argument("--remote-latency", type=int, default=140, help="Remote latency value")
...
elif buildEnv["PROTOCOL"] == "MESI_Three_Level":
    from protocols.generatedsynthesismesithreelevelNUMA import make_l1_cache, make_dir, make_l2_private_cache, make_l2_cache, has_l2_cache
elif buildEnv["PROTOCOL"] == "nicolo-MSI_unord-ThreeLevels":
...
# elif buildEnv["PROTOCOL"] == "MESI_unord_nicolo":
#     from protocols.generated_nicolo import make_l1_cache, make_dir, make_l2_cache, make_l2_private_cache, has_l2_cache
...
class MyNetwork(GarnetNetwork):
    ...
    def connect_routers(self, routers, latency=8):
        for ri in routers:
            for rj in routers:
                if ri == rj:
                    continue  # Don't connect a router to itself!
                self.int_links.append(
                    GarnetIntLink(link_id=len(self.int_links), src_node=ri, dst_node=rj, latency=latency)
                )
...
# system.cpu = [X86TimingSimpleCPU(cpu_id=i) for i in range(args.cores)]
...
number_of_cores_per_node = args.cores
...
for cluster_id, cluster in enumerate(split_in_half(range(args.cores))):
    ...
    if args.remote_memory:
        ...
        network.connect_routers([dir_rounter, cluster_router], remote_latency)
    ...
    if has_l2_cache():
        ...
        # network.connect_controller(l2_router, l2_cache, 10)
    
    
    for j in cluster:
        cpu = O3CPU(cpu_id=j)
        # cpu = X86O3CPU(cpu_id=j)
        cpu.createInterruptController()
        cpus.append(cpu)
    
        l1cache = make_l1_cache(system, ruby_system, cpu, j)
        exec("ruby_system.l1_cntrl%d = l1cache" % j)
        l1router = network.create_router()
        network.connect_controller(l1router, l1cache, latency = 10)
        l2_private_cache = make_l2_private_cache(system, ruby_system, cluster_id)
        exec("ruby_system.l2_private_cntrl%d = l2_private_cache" % j)
        l2_router_private = network.create_router()
        network.connect_controller(l2_router_private, l2_private_cache, latency = 10)
        component_router.append(l2_router_private)
        network.connect_routers([l1router, l2_router_private], latency=10)
        network.connect_routers([l2_router_private, l2_router], latency=10)
    ...
    # network.connect_routers(component_router, latency=10)
    # network.connect_routers(slow_routers, latency=10)
    # network.connect_routers([component_router, l2_router], latency=10)
    network.connect_routers([l2_router,cluster_router], latency=10)
    if len(fast_routers) > 0:
        network.connect_routers(fast_routers, latency=10)
